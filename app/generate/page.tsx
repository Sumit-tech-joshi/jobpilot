...
function ResumeView({ resume, keywords, highlightOn }: {
  ...
  return (
    <div className="space-y-5 text-sm">
      ...
      {resume.summary && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-widest text-[#8B5CF6] mb-1">Summary</h3>
      ...
      {resume.skillSections?.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-widest text-[#8B5CF6] mb-2">Skills</h3>
          <div className="space-y-1">
            {resume.skillSections.map((s, i) => (
              <div key={i} className="flex gap-2 flex-wrap">
                <span className="text-[#94A3B8] font-medium min-w-[120px]">{s.categoryName}:</span>
      ...
      {resume.experience?.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-widest text-[#8B5CF6] mb-2">Experience</h3>
              ...
              <div key={i}>
                <div className="flex justify-between items-start flex-wrap gap-1">
                  <div>
                    ...
                    <span className="text-[#94A3B8]"> · {exp.employer}</span>
                    {exp.location && <span className="text-[#94A3B8]">, {exp.location}</span>}
                  </div>
                  <span className="text-[#94A3B8] text-xs">{exp.dates}</span>
                </div>
      ...
      {resume.education?.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-widest text-[#8B5CF6] mb-2">Education</h3>
            ...
            {resume.education.map((e, i) => (
              <div key={i} className="flex justify-between flex-wrap gap-1">
                <div>
                  ...
                  <span className="text-[#94A3B8]"> · {e.institution}</span>
                  {e.location && <span className="text-[#94A3B8]">, {e.location}</span>}
                  {e.notes && <p className="text-[#94A3B8] text-xs mt-0.5">{e.notes}</p>}
                </div>
                <span className="text-[#94A3B8] text-xs">{e.dates}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {resume.certifications?.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-widest text-[#8B5CF6] mb-2">Certifications</h3>
          <div className="space-y-1">
            {resume.certifications.map((c, i) => (
              <div key={i} className="flex justify-between flex-wrap gap-1">
                <span className="text-[#cdd9e5]">{c.name} <span className="text-[#94A3B8]">· {c.issuingBody}</span></span>
                <span className="text-[#94A3B8] text-xs">{c.dates}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {resume.projects?.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-widest text-[#8B5CF6] mb-2">Projects</h3>
          <div className="space-y-3">
            {resume.projects.map((p, i) => (
              <div key={i}>
                <p className="font-semibold text-white">{p.name}</p>
                {p.description && <p className="text-[#94A3B8] text-xs" dangerouslySetInnerHTML={hl(p.description)} />}
  ...
  return (
    <div className="min-h-screen bg-[#080C14]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-white">Document <span className="bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] bg-clip-text text-transparent">Generator</span></h1>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                {analysing && (
                  <span className="flex items-center gap-1.5 text-xs text-[#8B5CF6]">
              ...
              <textarea
                ...
                rows={10}
                className="w-full bg-[#080C14] border border-white/[0.07] rounded-lg px-3 py-2.5 text-sm text-[#cdd9e5] placeholder-[#475569] resize-y focus:outline-none focus:border-[#6366F1]"
              />
            </div>
              ...
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5 space-y-4">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#94A3B8] text-sm">{analysis.company}{analysis.location ? ` · ${analysis.location}` : ''}</p>
                    <div className="flex gap-2 flex-wrap mt-1.5">
                      {analysis.experienceLevel && (
                        <span className="px-2 py-0.5 bg-[#6366F1]/15 border border-[#6366F1]/40 rounded-full text-xs text-[#8B5CF6] capitalize">{analysis.experienceLevel}</span>
                      )}
                      {analysis.employmentType && analysis.employmentType !== 'unknown' && (
                        <span className="px-2 py-0.5 bg-white/[0.03] border border-white/[0.06] rounded-full text-xs text-[#94A3B8] capitalize">{analysis.employmentType}</span>
                      )}
                      {analysis.salary && (
                        <span className="px-2 py-0.5 bg-white/[0.03] border border-white/[0.06] rounded-full text-xs text-[#94A3B8]">{analysis.salary}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      {analysis.skillsYouHave.length === 0 && <span className="text-xs text-[#475569]">None matched</span>}
                    </div>
                  </div>
                  <div>
                ...
                {analysis.niceToHave.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-[#94A3B8] mb-1.5">Nice to have</p>
                    <div className="flex flex-wrap gap-1">
                      {analysis.niceToHave.map(s => (
                        <span key={s} className="px-1.5 py-0.5 bg-white/[0.03] border border-white/[0.06] rounded text-xs text-[#94A3B8]">{s}</span>
                      ))}
                    </div>
                  </div>
                )}
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5">
              <p className="text-sm font-medium text-white mb-3">Job Details <span className="text-[#475569] font-normal">(auto-filled from analysis)</span></p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#080C14] border border-white/[0.07] rounded-lg px-3 py-2 text-sm text-[#cdd9e5] placeholder-[#475569] focus:outline-none focus:border-[#6366F1]"
                  />
                </div>
                <div>
                  <label className="text-xs text-[#94A3B8] mb-1 block">Company</label>
                  <input
                    value={manualCompany}
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm border transition-all ${
                      tone === t
                        ...
                        : 'bg-[#080C14] border-white/[0.07] text-[#94A3B8] hover:border-[#6366F1]/40 hover:text-white'
                    }`}
                  >
                    {TONE_LABELS[t]}
                  </button>
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5 space-y-4">
              ...
              <div className="flex rounded-xl overflow-hidden border border-white/[0.07]">
                {(['resume', 'coverletter', 'both'] as const).map((t, i) => (
                  <button
                    ...
                    } ${generateType === t ? 'bg-[#6366F1] text-white' : 'bg-[#080C14] text-[#94A3B8] hover:text-white'}`}
                  >
              ...
              <button
                ...
                className="w-full py-3 bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#6366F1]/20"
              >
                {generating ? (
                  <>
          ...
          <div className="space-y-4">
            {!hasOutput && !generating && (
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-10 text-center">
                <div className="w-14 h-14 rounded-full bg-[#6366F1]/10 border border-[#6366F1]/20 flex items-center justify-center mx-auto mb-4">
                ...
                <p className="text-[#94A3B8] text-sm">Paste a job description and click Generate.</p>
              </div>
            )}

            {generating && (
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-10 text-center">
                <div className="w-14 h-14 rounded-full bg-[#6366F1]/10 border border-[#6366F1]/20 flex items-center justify-center mx-auto mb-4 animate-pulse">
                  <svg className="w-7 h-7 text-[#8B5CF6]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </div>
                <p className="text-white font-medium mb-1">Generating your documents…</p>
                <p className="text-[#94A3B8] text-sm">This usually takes 10–20 seconds.</p>
              </div>
            )}
            ...
            {hasOutput && !generating && (
              <>
                {/* Toolbar */}
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-3 flex items-center gap-2 flex-wrap">
                  {/* Tabs */}
                  {resume && coverLetter && (
                    <div className="flex rounded-lg overflow-hidden border border-white/[0.07] mr-auto">
                      <button onClick={() => setActiveTab('resume')}
                        className={`px-3 py-1.5 text-xs font-medium transition-colors ${activeTab === 'resume' ? 'bg-[#6366F1] text-white' : 'bg-[#080C14] text-[#94A3B8] hover:text-white'}`}>
                        Resume
                      </button>
                      <button onClick={() => setActiveTab('coverletter')}
                        className={`px-3 py-1.5 text-xs font-medium border-l border-white/[0.07] transition-colors ${activeTab === 'coverletter' ? 'bg-[#6366F1] text-white' : 'bg-[#080C14] text-[#94A3B8] hover:text-white'}`}>
                        Cover Letter
                      </button>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg border transition-colors ${highlightOn ? 'bg-yellow-400/20 border-yellow-400/40 text-yellow-300' : 'border-white/[0.07] text-[#94A3B8] hover:text-white hover:border-white/[0.2]'}`}
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  {versions.length > 1 && (
                    <div className="relative">
                      <button
                        ...
                        className="p-1.5 rounded-lg border border-white/[0.07] text-[#94A3B8] hover:text-white hover:border-white/[0.2] transition-colors"
                      ...
                      {showVersions && (
                        <div className="absolute right-0 top-full mt-1 w-52 bg-white/[0.02] border border-white/[0.06] rounded-2xl shadow-xl z-20 overflow-hidden">
                          {versions.map(v => (
                            <button key={v.id} onClick={() => loadVersion(v)}
                              className="w-full text-left px-3 py-2.5 text-xs hover:bg-white/[0.03] transition-colors border-b border-white/[0.05] last:border-0">
                              <p className="text-white font-medium">{v.label}</p>
                              <p className="text-[#475569]">{v.timestamp.toLocaleTimeString()}</p>
                            </button>
                          ))}
                        </div>
                      )}
                ...
                <div className="flex gap-2 flex-wrap">
                  {(Object.keys(TWEAK_LABELS) as Tweak[]).map(tw => (
                    <button key={tw} onClick={() => generate(tw)} disabled={generating}
                      className="px-3 py-1.5 bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.06] rounded-full text-xs text-[#94A3B8] hover:text-white hover:border-[#6366F1]/40 transition-all disabled:opacity-40">
                      ↺ {TWEAK_LABELS[tw]}
                    </button>
                  ))}
                </div>
                    ...
                    <div>
                      <div className="flex justify-end mb-3">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-white/[0.07] rounded-xl text-[#94A3B8] hover:text-white hover:border-white/[0.2] transition-colors"
                        >
                          {editingCL ? (
                            <>
                      ...
                      {editingCL ? (
                        <textarea
                          ...
                          rows={20}
                          className="w-full bg-[#080C14] border border-[#6366F1]/40 rounded-xl px-3 py-2.5 text-sm text-[#cdd9e5] focus:outline-none resize-y leading-relaxed"
                        />
                      ) : (
                    ...
                    <>
                      {(['pdf', 'docx'] as const).map(fmt => (
                        <button key={fmt}
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 bg-white/[0.02] border border-white/[0.07] rounded-xl text-xs text-[#94A3B8] hover:text-white hover:border-white/[0.2] transition-colors"
                        >
...
function SaveModal({ jobTitle, company, resume, coverLetter, onClose }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#0D1424] border border-white/[0.07] rounded-2xl p-6 w-full max-w-md shadow-2xl shadow-black/60">
        {saved ? (
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={title} onChange={e => setTitle(e.target.value)}
                  className="w-full bg-[#080C14] border border-white/[0.07] rounded-lg px-3 py-2 text-sm text-[#cdd9e5] focus:outline-none focus:border-[#6366F1]"
                />
              </div>
              <div>
                <label className="text-xs text-[#94A3B8] mb-1 block">Company</label>
              ...
              <div className="flex gap-2 text-xs text-[#94A3B8]">
                {resume && <span className="px-2 py-1 bg-white/[0.04] rounded">Resume attached</span>}
                {coverLetter && <span className="px-2 py-1 bg-white/[0.04] rounded">Cover letter attached</span>}
              </div>
              {error && <p className="text-red-400 text-xs">{error}</p>}
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={onClose}
                className="flex-1 py-2 border border-white/[0.07] rounded-xl text-sm text-[#94A3B8] hover:text-white transition-colors">
                Cancel
              </button>
              <button onClick={handleSave} disabled={saving}
                className="flex-1 py-2 bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] hover:opacity-90 disabled:opacity-50 text-white text-sm font-medium rounded-xl transition-all flex items-center justify-center gap-2">
                {saving ? (
